...
if ($_SERVER['REQUEST_METHOD'] == "POST"){
    ...
    $ccondition = (trim($_POST['ccondition']));
...
*/
    ...
    if (empty($ccondition)){
        $errccondition = "<span class='error'>Condition is required</span>";
        $errmsg = 1;
    }
    if (empty($cprice)){
...
* ***********************************************************************
*/
    $sql = "SELECT * FROM csci490db WHERE cname = ?";
...
*/
        try {
            $sql = "INSERT INTO csci490db (`cname`, `ccondition`, `cprice`, `cquantity`)
                   VALUES (:cardName, :cardCondition, :cardPrice, cardQuantity, :inputDate)";
            $stmt = $pdo->prepare($sql);
            $stmt->bindValue('cardName', $cname);
            $stmt->bindValue('cardCondition', $ccondition);
            $stmt->bindValue('cardPrice', $cprice);
            $stmt->bindValue('cardQuantity', $cquantity);
...
if ($showform == 1){
    ...
    <fieldset>
        ...
        <table>
            ...
            <tr>
                <?php
                if (isset($errccondition)){
                    echo $errccondition . "<br>";
                }
                ?>
                <th><label for="ccondition"> Card Condition: </label></th>
                <td><input type="text" class="form-control" name="ccondition" id="ccondition" placeholder="Good/Great/Pristine/Etc"
                           value="<?php if (isset($ccondition)) {echo $ccondition;}?>"</td>
            </tr>
            <tr>
                <?php
                if (isset($errcprice)){
